                                              ...
                                              wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    uint8_t ucaDefaultName[MAX_BT_DEVICE_NAME_LENGTH] = "Baton";
    uint8_t local_bd_address[6];
    bool bStart;
    BTStatus_t xStatus;
    ...
    switch ( event )
    {
        ...
        case BTM_ENABLED_EVT:
            ...
            {
                /* Get Bluetooth device name from Amazon ACE layer using platform hook.
                 * This is a hack that Amazon wants us to do for Baton.
                 * They could have used BTSetDeviceProperty from their app to set the device name,
                 * but they insisted that we do it in the stack. */
                if(pxPlatformDevPropInterface  != NULL && pxPlatformDevPropInterface->get_bd_device_name != NULL)
                {
                    pxPlatformDevPropInterface->get_bd_device_name(&ucaDefaultName[0], sizeof(ucaDefaultName));
                }

                //Set device name
                if( btsnd_hcic_change_name( (uint8_t *)ucaDefaultName ) )
                {
                    ...
                    memcpy(xWICEDPropCache.pucDevName, ucaDefaultName, sizeof(ucaDefaultName));
                }

                /* Get public BD address from Amazon ACE layer using platform hook and update
                 * the 43012 controller. This is a hack that Amazon wants us to do for Baton.
                 * They could have used BTSetDeviceProperty from their app to set the BD address,
                 * but they insisted that we do it in the stack. */
                if(pxPlatformDevPropInterface != NULL && pxPlatformDevPropInterface->get_bd_address != NULL)
                {
                    if(WICED_SUCCESS == pxPlatformDevPropInterface->get_bd_address(&local_bd_address[0], sizeof(local_bd_address), PLATFORM_BD_ADDRESS_PUBLIC))
                    {
                        wiced_bt_set_local_bdaddr((uint8_t *)&local_bd_address[0]);
                    }
                }
        ...
        case BTM_PAIRING_COMPLETE_EVT:
            ...
            //@TODO: revisit this to handle pairing failure as well
            //@TODO: Move this callback to BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT (here for testing purpose only)
            if ( BT_TRANSPORT_LE == p_event_data->pairing_complete.transport )
            {
                if ( xBTCallbacks.pxBondedCb )
                    xBTCallbacks.pxBondedCb( eBTStatusSuccess, (BTBdaddr_t *) p_event_data->pairing_complete.bd_addr, true );
            }
            break;
        ...
        case BTM_ENCRYPTION_STATUS_EVT:
            if (!bSecurityReqReceived)
            {
                ...
                //@TODO: revisit this to handle pairing failure as well
                //@TODO: Move this callback to BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT (here for testing purpose only)
                if (BT_TRANSPORT_LE == p_event_data->encryption_status.transport)
                {
                    if (xBTCallbacks.pxBondedCb)
                        xBTCallbacks.pxBondedCb(eBTStatusSuccess, (BTBdaddr_t *)p_event_data->encryption_status.bd_addr, true);
                }
            }
            break;
...
#if 1  /* Amazon informed that pxBondedCb will be deprecated and we should only use pxPairingStateChangedCb going forward */
    if (xBTCallbacks.pxBondedCb)
    {
