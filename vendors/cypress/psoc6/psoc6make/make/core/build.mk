...
$(1)_shared_lib:
	@cat $$($(1)_SHAREDLIB_BUILD_LOCATION)/inclist.rsp | sed -e "s/I\.\//I$$($(1)_SED_PATTERN)\//g" | tr " " "\n"\
		> $$($(1)_SHAREDLIB_BUILD_LOCATION)/inclist_export.rsp ;\
	 cat $$($(1)_SHAREDLIB_BUILD_LOCATION)/liblist.rsp | sed -e "s/\.\//$$($(1)_SED_PATTERN)\//g" | tr " " "\n"\
		> $$($(1)_SHAREDLIB_BUILD_LOCATION)/liblist_export.rsp;\
	 if [ -s $$($(1)_SHAREDLIB_BUILD_LOCATION)/artifact.rsp ]; then\
	 	sed '1s;^;$$($(1)_SHAREDLIB_BUILD_LOCATION)/;' $$($(1)_SHAREDLIB_BUILD_LOCATION)/artifact.rsp > $$($(1)_SHAREDLIB_BUILD_LOCATION)/artifact_export.rsp;\
	 else\
		touch $$($(1)_SHAREDLIB_BUILD_LOCATION)/artifact_export.rsp;\
	 fi
...
$(CY_BUILD_ALL_OBJ_FILES): $(CY_CONFIG_DIR)/.cycompiler

#
# Dependencies for link
#
$(CY_BUILD_TARGET): | CY_BUILD_precompile
$(CY_BUILD_TARGET): $(CY_CONFIG_DIR)/.cylinker
